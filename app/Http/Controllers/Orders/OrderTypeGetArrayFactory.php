    ...
    public function getChildFields($childKey, $orderCommonId)
    {
        ...
        switch ($childKey) {
            ...
            case 'salaryAddition':
                ...
                $childFields = $this->getSalaryAdditionFields($orderCommonId);
                break;

            case 'vacationRecall':

                $childFields = $this->getVacationRecallFields($orderCommonId);
                break;
            case 'damage':

                $childFields = $this->getDamageFields($orderCommonId);
                break;
    ...
    private function getBusinessTripFields($orderCommonId)
    {
        ...
        for ($i = 0; $i < count($childData['data']); $i++) {
            ...
            $fields[] = [
                ...
                'regionId' => $childData['data'][$i]->listRegionsId->id
...
//        }
    ...
    private function getDismissalFields($orderCommonId)
    {
        ...
        for ($i = 0; $i < count($childData['data']); $i++) {
            ...
            {
                foreach ($dismissalNotes['data'] as $note)
                {
                    $notes[] = [
                        'id'   => $note->id,
                        'note' => $note->note
...
//            'dayOffId' => $childData['data'][0]->dayOffId->id
    ...
    private function  getWarningFields($orderCommonId)
    {
        ...
        if($childData['totalCount'] !=0){
            ...
            foreach ($childData['data'] as $key => $single_data){
                ...
                if(isset($single_data->positionId) && $single_data->positionId != null){
                    $fields[0]['userId'][$key]['posName']   = $single_data->positionId->posNameId->name;
                    $fields[0]['userId'][$key]['strName']   = $single_data->positionId->structureId->name;
                }else{
                    $fields[0]['userId'][$key]['posName'] = '';
                    $fields[0]['userId'][$key]['strName'] = '';
                }
    ...
    private function getStaffCancellationFields($orderCommonId){
        ...
        if(!isset($posSalary->data[0]->value)){

            $fields['order']['salary']   = 0;
        }
        else {
    ...
    /**
     * prepare reward view array
     * @param $orderCommonId
     * @return array
     */
    private function getSalaryAdditionFields($orderCommonId)
    {
        $fields = [];
        ...
        $fields['ordersalaryaddition']=$dataOrderSalaryAddition;

        $dataRelUserPayments= Service::request([
            'method' => 'GET',
            'url' => Service::url('hr', 'crud'),
            'params' => [
                'sc' => ['id', 'isPercent', 'valus', 'userId', 'startDate'],
                'filter' => '"id" : { "=" : "' . $dataOrderSalaryAddition->data[0]->relUserPaymentsIdId . '" }'
            ],
            'options' => [
                'headers' => [
        ...
        $userId=$dataRelUserPayments->data[0]->userId->id;
        $fields['reluserpayment']=$dataRelUserPayments;

        $fields['userInfo']=UserPositionController::getPosByUserId($userId, 'raw');


        return $fields;
    }

    private function getVacationRecallFields($orderCommonId){

        $fields = [];

//        $fields[0]=[];
        $dataRecall= Service::request([
            'method' => 'GET',
            'url' => Service::url('hr', 'crud'),
            'params' => [
                'sc' => ['id', 'orderVacationId', 'returnDate', 'listReturnTypeId.id', 'listReturnTypeId.label'],
                'filter' => '"orderCommonId.id" : { "=" : "' . $orderCommonId . '"}'
...
//        dd($dataRecall);
        $fields['main'][]['OrderVacationReturn']=$dataRecall;

        $dataOrderVacationReplacement= Service::request([
            'method' => 'GET',
            'url' => Service::url('hr', 'crud'),
            'params' => [
                'sc' => ['id', 'orderVacationReturnId.id', 'startDate', 'endDate'],
                'filter' => '"orderVacationReturnId.id" : {"=" : "' . $dataRecall->data[0]->id . '"}'
            ],
            'options' => [
                'headers' => [
        ...
        $fields['main'][]['OrderVacationReplacement']=$dataOrderVacationReplacement;
        $listReturnTypes= Service::request([
            'method' => 'GET',
            'url' => Service::url('hr', 'crud'),
            'params' => [
                'sc' => ['id', 'name', 'label'],
        ...
        $fields['main'][]['listReturnTypes']=$listReturnTypes;

        return $fields;
    }

    private function getDamageFields($orderCommonId){

        $fields = [];

        $dataDamage= Service::request([
            'method' => 'GET',
            'url' => Service::url('hr', 'crud'),
            'params' => [
                'sc' => ['id', 'orderCommonId', 'valueTotal', 'relUserPaymentsId', 'positionId'],
                'filter' => '"orderCommonId.id" : { "=" : "' . $orderCommonId . '"}'
        ...
        $fields['RelUserDamage']=$dataDamage;
    ...
    private function getRewardFields($orderCommonId)
    {
        ...
        $positionController             = new UserPositionController();
        ...
        if (isset($userRewardIndividuals->totalCount) && $userRewardIndividuals->totalCount > 0 )
        {
            ...
            {
                $position = $positionController->getPosByUserId2($userRewardIndividual->userIdId);
                ...
                {
                    $structureName = $position->positionId->structureId->name;
                    $positionId    = $position->positionId->id;
                    $positionName  = $position->positionId->posNameId->name;
                }
