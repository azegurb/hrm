...
(function(document, window, $) {
    $.fn.uploaderC = function(id){
        var wrapper     = '#'+id;
        var input       = '#'+$(wrapper).find('input').prop('id');
        var container   = $(wrapper).find('.file-wrap').find('div');
        'use strict';
        ...
        $(wrapper).fileupload({
            ...
            if ($(e.target).parents('.file-item-wrap').length === 0) $(input).trigger('click');
        });
